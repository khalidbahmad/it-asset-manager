...
function DonutChart() {
    ...
    const counts = useMemo(() => {
        const c = { Disponible: 0, Affecté: 0, 'En réparation': 0, Retraité: 0 };
        assets.forEach(a => {
            const statusName = a.status?.name ?? '';
            if (c[statusName] !== undefined) c[statusName]++;
        });
        return c;
    }, [assets]);
    ...
    const data = [
        { val: counts['Disponible'],    color: '#4f8ef7', label: 'Disponible'    },
        { val: counts['Affecté'],       color: '#38d9a9', label: 'Affecté'       },
        { val: counts['En réparation'], color: '#f6a623', label: 'En réparation' },
        { val: counts['Retraité'],      color: '#ff5c5c', label: 'Retraité'      },
    ];
